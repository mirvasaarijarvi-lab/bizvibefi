...
The `Release candidate smoke` workflow (`.github/workflows/rc-smoke.yml`) runs `scripts/security/checklist-smoke.sh` on every published prerelease (or manually via workflow dispatch). It executes the SQL assertions from `docs/SECURITY_CHECKLIST.md` Section 0 — GRANT/EXECUTE introspection plus `42501` expectations for `anon`, `authenticated`, and `service_role` — then chains the signed-URL Vitest suite. Logs are uploaded as `security-smoke-logs` (14-day retention).
