...
import { Rocket, Users, Zap, ArrowRight, Wrench, Search, Handshake, TrendingUp, CalendarCheck, Code2, Lightbulb, MessageSquare, Gem, Calendar, MapPin } from "lucide-react";
import { format } from "date-fns";
import { fi as fiLocale, enUS, sv as svLocale } from "date-fns/locale";
...
const NextEventTeaser = () => {
  ...
  const locale = lang === "fi" ? fiLocale : lang === "sv" ? svLocale : enUS;

  const { data: event } = useQuery({
    queryKey: ["next-upcoming-event", lang],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("events")
        .select("id, title, title_fi, title_sv, location, location_fi, location_sv, starts_at")
  ...
  const title =
    (lang === "fi" && event.title_fi) ||
    (lang === "sv" && event.title_sv) ||
    event.title;
  const location =
    (lang === "fi" && event.location_fi) ||
    (lang === "sv" && event.location_sv) ||
    event.location;
...
const Index = () => {
  ...
  return (
    <Layout>
      <PageMeta title="<Good Vibes Café/> — The Collective for Builders" description="A collective of builders, strategists and connectors. Shoot first, fail spectacularly, ship solid." />
      {/* Hero */}
      <section className="relative overflow-hidden py-24 md:py-36">
        ...
        <div className="container relative">
          ...
          >
            <HeroAvatar size="lg" alt="Good Vibes Café mascot waving a welcome to the builder collective" />
            ...
            <p className="font-display text-xl md:text-3xl font-bold tracking-[-0.01em] mt-2 text-foreground">
              The Collective for Builders
            </p>
...
const VibetorTierCard = () => {
  ...
  return (
    ...
    >
      ...
      <Button variant="hero" className="mt-8 w-full" asChild>
        <Link to="/apply-vibetor">{t("tiers.vibetor.cta")}</Link>
      </Button>
    </motion.div>
  );
};
